...
class trotGait:
    ...
    def stepTrajectory(self , phi , V , angle , Wrot , centerToFoot , stepOffset = 0.5): #phi belong [0,1), angles in degrees
        if (phi >= 1):
            phi = phi - 1.
    ...
    def loop(self , V , angle , Wrot , T , offset , bodytoFeet_ , step_asym=0.0 , duty_cycle=0.5):
        ...
        stepOffset = np.clip(duty_cycle * np.exp(step_asym), 0.1, 0.9)

        step_coord = self.stepTrajectory(self.phi + offset[0] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[0,:])), stepOffset) #FR
        ...
        step_coord = self.stepTrajectory(self.phi + offset[1] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[1,:])), stepOffset)#FL
        ...
        step_coord = self.stepTrajectory(self.phi + offset[2] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[2,:])), stepOffset)#BR
        ...
        step_coord = self.stepTrajectory(self.phi + offset[3] , V , angle , Wrot , np.squeeze(np.asarray(bodytoFeet_[3,:])), stepOffset)#BL
